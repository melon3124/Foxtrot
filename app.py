...
if st.session_state.mode == "class" and cls:
    ...
    if name_clean:
       ...
       # Assuming the rest of your app's code is above this section...
        ...
        with t5:
            try:
                # Assuming 'role' and 'cls' are defined earlier in your script
                # Assuming 'name_clean' and 'name_disp' are defined as the selected cadet's name
                if role == "admin":
                    st.subheader("Cadet Touring Status")
                    ...
                    if conduct_df.empty or conduct_name_col is None:
                        ...
                        row_conduct = conduct_df[conduct_df["NAME_CLEANED"] == name_clean]
                        
                        current_status = "Not Touring"
                        if not row_conduct.empty and "TOURING STATUS" in row_conduct.columns:
                            current_status = row_conduct.iloc[0]["TOURING STATUS"] or "Not Touring"

                        new_status = st.selectbox(
                            "Select Touring Status",
                            ["Not Touring", "Touring"],
                            index=["Not Touring", "Touring"].index(current_status),
                            help="Update the cadet's touring status"
                        )

                        if st.button("📤 Save Touring Status"):
                            try:
                                # Fetch the worksheet
                                ...
                                # Add the TOURING STATUS column if it doesn't exist
                                ...
                                for row in body:
                                    if clean_cadet_name_for_comparison(row[name_idx]) == name_clean:
                                        row[touring_status_idx] = new_status
                                        updated = True
                                        break
                                ...
                                if not updated:
                                    ...
                                    new_row[touring_status_idx] = new_status
                                    body.append(new_row)

                                ws.clear()
                                ws.update(f"A1:{chr(64 + len(headers))}{len(body) + 1}", [headers] + body)
                                st.cache_data.clear()
                                st.success(f"✅ Touring status for {name_disp} saved successfully!")
                            except Exception as e:
